...
var _compare = function _compare(str1, str2) {
...
exports.default = {
	...
	getMonthName: function getMonthName(monthNumber, declination, countChar) {
	...
	getCurrentWeekDates: function getCurrentWeekDates(date) {
		var now = date;
	...
	getLastWeekDates: function getLastWeekDates(date) {
		var now = date;
		var start = new Date(now.getFullYear(), now.getMonth(), now.getDate());
		var delta = start.getDay() > 0 ? 1 : -6;
	...
	getCurrentMonthDates: function getCurrentMonthDates(date) {
		var now = date;
	...
	getLastMonthDates: function getLastMonthDates(date) {
		var now = date;
	...
	getCurrentQuarterDates: function getCurrentQuarterDates(date) {
		var now = date,
		    start = '',
		    stop = '';
	...
	getLast30DaysDates: function getLast30DaysDates(date) {
		var now = date;
		var start = new Date(now.getFullYear(), now.getMonth(), now.getDate());
	...
	getCurrentYearDates: function getCurrentYearDates(date) {
		var now = date;
	...
	countCalendarDayBetween: function countCalendarDayBetween(date1, date2) {
	...
	isYesterday: function isYesterday(now, date) {
		return now > date && this.countCalendarDayBetween(now, date) == 1;
	},
	countHourBetween: function countHourBetween(date1, date2) {
	...
	dateFormat: function dateFormat(date, format, alternativeFormat, dateTimeNow) {
		var mask;
		var prefix = '';
	...
	parseDate: function parseDate(vDate, vFormat) {
		...
		var vSettings = {
			...
			monthsShort: self.i18n.ru.months.map(function (n) {
